...
- `OVERHEAR_INSECURE_NO_KEYCHAIN=1` — CI-only escape hatch that skips the Keychain and uses an in-memory encryption key for transcripts. The explicit override is enough on its own; the CI/GitHub flags also enable the bypass. Never set this in production (release builds abort if this is present).
